...
export default function LibraryScreen() {
  const { favorites, toggleFavorite, books } = useContext(FavoritesContext);
          ...
          <View style={styles.infoCol}>
            ...
            <View style={styles.cardActions}>
              ...
              <IconButton
                ...
                iconColor={isFavorite ? '#7F7FD5' : '#b0b5c9'}
              />
            </View>
          </View>
        </View>
  ...
  return (
    <View style={styles.root}>
      <View style={styles.headerRow}>
        <Title style={styles.title}>{i18n.t('library')}</Title>
        <MaterialCommunityIcons name="bookshelf" size={28} color="#7F7FD5" />
      </View>
      {(!books || books.length === 0) ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="book-off-outline" size={64} color="#d1d5e6" />
          <Text style={styles.emptyText}>{i18n.t('no_books')}</Text>
        </View>
      ) : (
        <FlatList
          data={books}
